...
#import "ZhiYingOrderResultViewController.h"
...
-(void)createUI{
    ...
    if (_isDuihuan){
        [footerView.commitOrderBtn setTitle:@"兑换" forState:normal];
        [footerView.commitOrderBtn addTarget:self action:@selector(duiHuanBtnClick) forControlEvents:UIControlEventTouchUpInside];
    }
    else {
        //提交订单
        [footerView.commitOrderBtn addTarget:self action:@selector(commitAction) forControlEvents:UIControlEventTouchUpInside];
    }
    footerView.totalPriceLb.text = [NSString stringWithFormat:@"￥%.2f",_goodsNum.floatValue * _goodsprice.floatValue];
...
-(void)duiHuanBtnClick{
    if (_dataDic) {
        if (_goodsNum.integerValue >0){
            ...
            self.progressHud.label.text = @"正在加载...";
            self.progressHud.mode = MBProgressHUDModeIndeterminate;
            [self.progressHud showAnimated:YES];
            [ZMCHttpTool postWithUrl:FBjiFenDuiHuanGoodsUrl parameters:parameters success:^(id responseObject) {
                if ([responseObject[@"status"] isEqualToString:@"1"]){
                    self.progressHud.mode = MBProgressHUDModeText;
                    self.progressHud.label.text = responseObject[@"msg"];
...
-(void)finishAction{
    ...
    if (tag == 100){
        //银行卡支付
        self.progressHud.mode = MBProgressHUDModeText;
        self.progressHud.label.text = @"敬请期待";
        [self.progressHud showAnimated:YES];
    ...
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(showOrderResultViewController) name:@"zhiYingPaySuccess" object:nil];
...
-(void)showOrderResultViewController{
    ZhiYingOrderResultViewController *orderVC = [[ZhiYingOrderResultViewController alloc]init];
    orderVC.goodsName = _goodsNum;
    orderVC.goodsThumb = _goodsThumbUrl;
    //缺少获取订单号
...
-(void)addBtnClick{
    ...
    _totalPriceLb.text = [NSString stringWithFormat:@"￥%.2f",_goodsNum.floatValue * _goodsprice.floatValue];
    [_tableView reloadData];
}
//减
...
-(void)dealloc{
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"zhiYingPaySuccess" object:nil];
}
