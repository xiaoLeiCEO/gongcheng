...
-(void)payMoneyBtnClick:(UIButton *)sender{
    if ([sender.currentTitle isEqualToString:@"付款"]){
        NSLog(@"%@",_dataSource[sender.tag - 100][@"pay_id"]);
        NSMutableDictionary *parameters = [[NSMutableDictionary alloc]init];
        parameters[@"token"] = [UserBean getUserDictionary][@"token"];
        parameters[@"order_id"] = _dataSource[sender.tag - 100][@"order_id"];
        NSLog(@"%@", parameters);
        [self.progressHud showAnimated:YES];
        if ([_dataSource[sender.tag - 100][@"pay_id"] isEqualToString:@"1"]){
            //支付宝支付
            [ZMCHttpTool postWithUrl:FBOrderPayUrl parameters:parameters success:^(id responseObject) {
                NSLog(@"%@", responseObject);
                if ([responseObject[@"status"] isEqualToString:@"1"]){
